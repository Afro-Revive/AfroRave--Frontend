...
### Added
- **Pagination fix on tickets tab** (`src/pages/creators/edit-event/tabs/tickets-tab.tsx`): Fixed broken `as PaginatedResponse<T> | []` casts (value used as type) — changed to `as PaginatedResponse<T> | undefined`. Moved `PaginatedResponse` import from `@/types` (not exported there) to `@/types/api`. Guarded all `.items` accesses with optional chaining and `?? []` / `?? 0` fallbacks.
- **Pagination fix on standalone page** (`src/pages/creators/standalone/index.tsx`): Cast `response.data` to `PaginatedResponse<EventData>` and access `.items` for the organizer events list. Cast `getEvent` response to `EventDetailData` (single-item endpoint). Fixed `event.desktopMedia` → `event.eventDetails.desktopMedia`.
- **Pagination fix on promo-code-form** (`src/pages/creators/add-event/ticket-forms/promo-code-form/promo-code-form.tsx`): Cast `createdPromoCodes.data` and `eventTickets.data` to `PaginatedResponse<PromoCodeData>` and `PaginatedResponse<TicketData>` respectively; access `.items` throughout.
