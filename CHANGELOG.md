...
### Added
- **Vendor applications page rebuilt on real data, grouped by event** (`src/pages/vendor/slots/index.tsx`, `src/pages/vendor/slots/components/vendor-application-card.tsx`): The page mapped `useGetVendorApplications()` results into cards with a broken trailing `event.` property access, fields that don't exist on `VendorApplications` (`name`/`date`/`secured_slots`/`total_slots`/`isEnded`/`id`), an undefined `handleMore`, a discarded `if (isLoading) { <Skeleton/> }`, and a link keyed off the application's own numeric `id` instead of `eventId`. Rebuilt as `VendorApplicationCard`, which fetches the real event via `useGetEvent(eventId)` (the applications endpoint has no image field) for the flyer/date/`isEnded`. A vendor with multiple applications to the same event (e.g. two Revenue slots, or a Revenue slot + a Service offer) previously rendered one near-duplicate card per application — added a `Map`-based `groupApplicationsByEvent` reducer so `VendorApplicationCard` now takes `applications: VendorApplications[]` for one event and sums `requestedSlots` across the group (`securedSlots` gated on `status === "Approved"`) into a single card.
- **`RequestVendorSlotModal`** (`src/pages/vendor/component/request-vendor-slot-modal.tsx`): New modal opened from the "Request For Slot/Offer" button on the vendor event-details cards, using the same chevron-picker technique as `ticket-resale.tsx`. Revenue vendors get an integer stepper for stall count (capped at `slotData.slotNumber`); Service vendors get a ₦-prefixed number input for their price offer (clamped to `serviceData.minBudget`–`maxBudget` when the listing has a budget range). Wired to the real `useApplyVendorSlot` mutation via a `VendorApplicationRequest` payload.
- **`vendor/slots/slot-details.tsx` wired to real data**: Was entirely on `MOCK_SLOTS` and a hardcoded `eventName`. Now reads `useGetVendorApplications()`, filters by the route's `eventId`, and derives each row's name/price via new `getSlotName`/`getSlotPriceLabel` helpers (Revenue → `slotData.slotName`/`price`; Service → `serviceData.serviceName`/budget range, via `formatNaira`). Removed the unused List/Map view toggle and "Add Filter" button; search now actually filters the list by name/category (previously typed into but never applied). Re-added `VendorSectionMap` as a static visual section.
- **Vendor slot card description truncates with a conditional "Show More"** (`src/pages/vendor/component/description-show-more.tsx`): `SlotDescription` clamps to 3 lines via `line-clamp-3`, comparing `scrollHeight` to `clientHeight` so the toggle only appears when the text actually overflows, instead of showing unconditionally.

### Fixed
- **`eventKeys.vendorApplications` was self-referential** (`src/lib/event-keys.ts`): `vendorApplications: () => [...eventKeys.vendorApplications, ...]` spread the function itself instead of `eventKeys.all`, which is what caused `useGetVendorApplications`'s "implicitly has return type 'any'" circular-reference error. Fixed to spread `eventKeys.all`, matching every other key in the object.
- **Stale price/stall count carried over between vendor slot cards** (`request-vendor-slot-modal.tsx`): The modal wasn't remounted when a different card's slot was opened (same component instance, only the `slot` prop changed), so `price`/`stallCount` state from the previously opened card lingered. Added a `prevVendorIdRef` that resets both back to their defaults during render whenever `slot.vendorId` changes. Also fixed the price `Input` being uncontrolled (no `value` prop), and `handleClose` resetting to `minBudget` instead of `0`.
- **Service vendor budget helper text only handled a budget *range*** (`request-vendor-slot-modal.tsx`): Showed `"Budget range: ₦X - ₦undefined"` whenever a listing had a single fixed `minBudget` with `hasBudgetRange: false`. Now branches on `hasBudgetRange` to show either the range or a plain `"Budget: ₦X"`.
- **Broken `.map()` syntax in `slot-details.tsx`**: `eventApplications.map((slot) => ( const isRevenue = ...; <div>...))` — a `const` declaration inside an implicit-return arrow expression, which isn't valid JS. Converted to a block body with an explicit `return`.

### Changed
- **Vendor event-details right column scrolls independently on desktop** (`event-details/index.tsx`): Added `md:max-h-[calc(100vh-140px)] md:overflow-y-auto` so a long list of wrapped slot cards scrolls within that column instead of growing the whole page; the left column (banner/event info/About) stays in place. Split the earlier `md:overflow-visible` into `md:overflow-x-visible md:overflow-y-auto` to keep the `md:min-w-0` flex-shrink fix intact (browsers force both axes to non-`visible` once either one isn't, which is what actually gives the flex item its automatic minimum size of `0`).
- **Vendor slot cards on the event-details page pin their action button to the bottom** (`event-details/index.tsx`): The "Request For Slot/Offer" button now has `mt-auto` inside the card's `flex flex-col`, so it sits at the bottom of a fixed-height card regardless of how much content sits above it.

### Added
- **`VendorSlot` type** (`src/types/vendor.ts`): Modeled directly off the real `GET /api/Event/{eventId}/vendor-slots` response shape — `vendorId`, `vendorType`, `category`, `vendorCategory`, `vendorName`, `status`, and a nested `vendorDetails` (`slotData` with `slotNumber`/`price`/`applicationDeadline`, `serviceData` with budget range/work hours/dates, `contact`, `hideSocialLinks`). Nothing resembling this shape previously existed on this branch.
- **Vendor event-details page now shows real vendor slots** (`src/pages/vendor/discover/event-details/index.tsx`): Previously rendered a single hardcoded card ("OUTDOOR FOOD STALL A", ₦200,000, 5 slots, "online only", Oct 20 2025) regardless of data. `useGetVendorSlots` was also mistyped as returning `VendorAvailableEventData[]` (an events shape) even though it hits `/api/Event/{eventId}/vendors`, which actually returns vendor slots — retyped to `VendorSlot[]`. Now `.map()`s over the real slots, rendering price-per-slot or budget range (via `formatNaira`), available slot count, work duration (Service), and application deadline per card; dropped the "Sales Channel" row since no backing field exists for it. Added loading/empty states.
- **Swipeable slot cards on mobile** (`event-details/index.tsx`): The cards row uses `flex-nowrap overflow-x-auto snap-x snap-mandatory` with each card `w-80 shrink-0 snap-center` on mobile — a touch-swipe carousel with scroll-snap, bleeding edge-to-edge via `-mx-6 px-6`. Reverts to the existing `flex-wrap` grid layout at `md:`.
- **`stripUnderscores` helper** (`src/lib/helper-func.ts`): Replaces underscores with spaces and capitalizes each word (`"food_beverage"` → `"Food Beverage"`), used to display `slot.category` on the vendor event-details cards.

### Changed
- **Vendor event-details page no longer renders inside the vendor sidebar layout** (`src/config/vendor-dashboard-routes.tsx`, `src/application.tsx`): `vendor_event_details` was nested under `VendorDashboardLayout` (sidebar + header) alongside the other vendor dashboard pages; moved it to a standalone top-level `<Route>` in `application.tsx` (same pattern already used for `vendor_slot_details`), still behind `VendorAuthGuard`, so it now renders full-page with no sidebar.

### Fixed
- **Vendor and vendor-facing routes were unprotected** (`src/config/vendor-dashboard-routes.tsx`, `src/config/creator-dashboard-routes.tsx`, `src/application.tsx`): Unlike `user_dashboard_routes` (`UserAuthGuard`) and the Organizer routes in `creator_dashboard_routes` (`OrganizerAuthGuard`), none of the five routes in `vendor_dashboard_routes` (profile, discover, wishlist, event details, slots) had any auth guard at all — an unauthenticated visitor could load them directly instead of being redirected. A `VendorAuthGuard` component already existed (`components/auth/vendor-auth-guard.tsx`) but was never wired in. Also fixed: five vendor-facing pages inside `creator_dashboard_routes` (`revenue_vendor`, `revenue_vendor_slot`, `service_vendor`, `individual_service_vendor`, `seating_maps`) were guarded with `OrganizerAuthGuard` under a `// Temporarily using OrganizerAuthGuard` comment, so real Vendor accounts got bounced with an "Access denied" toast; switched to `VendorAuthGuard`. `vendor_slot_details` (registered standalone in `application.tsx`, not via the routes file) had the same no-guard gap and is now wrapped too.
- **`profile-section.tsx` / `view-profile-modal.tsx` type errors from wrong `User` field paths** (`src/pages/vendor/profile/components/profile-section.tsx`, `src/pages/vendor/profile/view-profile-modal.tsx`): Both files referenced fields that don't exist on `User` (`user.socialLinks`, `user.portfolio`, `user.telphone`, `user.description`) instead of their real nested locations (`user.profile.businessData.socials`, `user.profile.businessData.portfolio.webUrl`, `user.profile.phoneNumber`). `profile-section.tsx`'s `isSocialLinksString`/`socialLinksObj` branch was dead code — `socials` is a fixed-shape object per the type, never a bare string — collapsed to a single `socials` var read directly off `user.profile.businessData.socials`. `view-profile-modal.tsx`'s gallery block referenced a non-existent `businessData.gallary` (typo) with a stray extra `)` breaking the JSX and an undefined loop var `i`; fixed to `.map` over the real `profile.gallery: string[]` field.
- **Event bookmark button showed stale text and no pending state** (`src/pages/landing-page/event-page/individual-event/_components/event-bookmark-button.tsx`): Label only updated after the mutation's `invalidateQueries` → refetch → prop round-trip. Added local `watchlisted` state (synced from the `isWatchlisted` prop via `useEffect`) that flips immediately on click and rolls back on mutation error. Also moved `cursor-pointer` from a pointless wrapper `<div>` onto the `<button>` itself, and added a spinning `Loader2` in place of the bookmark icon (button `disabled` + dimmed) while either mutation is in flight.
- **`SelectTrigger` padding not affecting size** (`src/components/ui/select.tsx`): Base classes hardcoded `data-[size=default]:h-9 data-[size=sm]:h-8` (fixed height), so extra `py-*` passed via `className` had no visible effect — the box couldn't grow past the fixed height. Changed to `min-h-9`/`min-h-8` so padding can expand the trigger while keeping the same default minimum size.

### Changed
- **`businessName` vs `companyName` on profile completion** (`src/pages/fans/complete-profile/index.tsx`): `onSubmit` previously sent both `businessName` and `companyName` set to the same value unconditionally. Now sends `businessName` only when `userAccountType === "Vendor"`, and `companyName` for every other account type (the other field is `undefined` in each case — both are optional on `CompleteProfileRequest`).
