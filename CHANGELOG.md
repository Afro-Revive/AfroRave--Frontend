...
### Added
- **Cart auto-open on ticket add** (`src/pages/landing-page/event-page/individual-event/sections/tickets.tsx`, `src/stores/index.ts`): `isCartOpen`, `openCart`, and `closeCart` added to `useCartStore` (not persisted). `createCart()` in `TicketCard` calls `useCartStore.getState().openCart()` immediately after `mutate()` — imperative Zustand access avoids hook dependency issues and microtask timing problems.
- **Cart modal slide-up animation** (`src/components/ui/dialog.tsx`, `src/components/reusable/base-modal.tsx`): Added `slideFromBottom` prop to `DialogContent` and `BaseModal`. When set, the dialog is anchored `bottom-0 left-0 right-0` and animates via `slide-in-from-bottom` / `slide-out-to-bottom` instead of the default center zoom.
- **Cart trigger UI inside `CartContainer`** (`src/pages/landing-page/event-page/cart/cart-container.tsx`): Checkout bar (shopping cart icon + ticket count pill + total price button) added at the bottom of the cart modal content. Total price and ticket count are computed from local store items crossed with `eventTickets` props. Calls the `action` prop (sync + open checkout) on click. `ShoppingCart` icon imported from lucide-react.
- **Vendor type selection on complete profile** (`src/pages/fans/complete-profile/index.tsx`): Two-button toggle ("REVENUE VENDOR" / "SERVICE VENDOR") with description text. Selected button gets filled white style; description text switches to `text-black/60` when active. Sets `vendorType` field via `setValue`. Error message shown below if field fails validation.
- **`businessName` field on complete profile** (`src/pages/fans/complete-profile/index.tsx`): Separate `businessName` field added alongside `companyName`. Label dynamically shows "BUSINESS NAME" for Vendor accounts and "COMPANY NAME" for Organizer accounts.
- **`BaseSelect` for category on complete profile** (`src/pages/fans/complete-profile/index.tsx`): Replaced raw shadcn `Select`/`SelectTrigger`/`SelectContent` with `BaseSelect`, matching the inset-label style of gender, country, and birthday selects (`editableSelectStyle` + `insetLabelStyle`).

### Fixed
- **Circular render crash in cart modal** (`src/pages/landing-page/event-page/cart/cart-container.tsx`): `CartContainer` was importing and rendering `CartTrigger`, which itself renders `Cart`, which renders `CartContainer` — creating an infinite component tree that crashed React. Fixed by removing `CartTrigger` from `CartContainer` entirely; the checkout action is now a self-contained button bar inside `CartContainer`.
- **Cart modal never mounting** (`src/pages/landing-page/event-page/individual-event/event-details.tsx`): `Cart` (the modal host) was not rendered in the component tree so `openCart()` had no subscriber. Fixed by mounting `<Cart event={event} />` directly in `EventDetails`.
...
- **Trending events pagination fix** (`src/pages/landing-page/home/own-the-stage.tsx`): Cast `trendingEventResponse.data` to `PaginatedResponse<TrendingEventData>` and access `.items` after changing `TrendingEventsResponse` from `ApiResponse<TrendingEventData[]>` to `ApiResponse<TrendingEventData>`.
- **Vendor select pagination fix** (`src/components/shared/vendor-select.tsx`): Cast `response.data` to `PaginatedResponse<EventData>` and access `.items` after fixing `EventsResponse` type.
