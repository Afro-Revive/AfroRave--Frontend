...
export function BasePieChart({
  ...
  const { slices, chartConfig, total } = useMemo(() => {
    ...
    const folded = ordered.slice(cap).reduce((sum, item) => sum + item.value, 0)
    ...
    const config: ChartConfig = {
      ...
      ...Object.fromEntries(
        resolved.map((slice) => [slice.key, { label: slice.label, color: slice.fill }]),
      ),
    }
  ...
  const chart = slices.length ? (
    <ChartContainer
      ...
      <PieChart>
        <ChartTooltip
          cursor={false}
          content={<ChartTooltipContent nameKey='key' hideLabel formatter={(value) => valueFormatter(Number(value))} />}
        />
        <Pie
          data={slices}
          dataKey='value'
